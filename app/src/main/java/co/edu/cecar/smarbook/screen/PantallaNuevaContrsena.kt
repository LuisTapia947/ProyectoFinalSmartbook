    ...
    Box(
        ...
        Column(
            ...
            Row(
                ...
                IconButton(onClick = { navegarAPantallaSolicitudRestablecimiento() }) {
                    Icon(
        ...
        Card(
            ...
            Column(
                ...
                Text(
                    text = "Ingresa el código enviado a tu correo y tu nueva contraseña",
                    fontSize = 12.sp,
                    color = Color.Gray,
                ...
                OutlinedTextField(
                    ...
                    supportingText = { if (errorCodigo.isNotEmpty()) Text(text = errorCodigo) },
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { 
                        nuevaContrasenaState = it
                        errorNuevaContrasena = ""
                    },
                    label = { Text("Nueva Contraseña") },
                    isError = errorNuevaContrasena.isNotEmpty(),
                    supportingText = { if (errorNuevaContrasena.isNotEmpty()) Text(text = errorNuevaContrasena) },
                    modifier = Modifier.fillMaxWidth(),
                    enabled = !cargando,
                ...
                OutlinedTextField(
                    ...
                    onValueChange = { 
                        confirmarContrsenaState = it
                        errorConfirmar = ""
                    },
                    label = { Text("Confirmar Contraseña") },
                    isError = errorConfirmar.isNotEmpty(),
                    supportingText = { if (errorConfirmar.isNotEmpty()) Text(text = errorConfirmar) },
                    modifier = Modifier.fillMaxWidth(),
                    enabled = !cargando,
                    visualTransformation = PasswordVisualTransformation(),
                    keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Password)
                ...
                Button(
                    onClick = {
                        ...
                        if (codigoState.isBlank()) errorCodigo = "Campo obligatorio"
                        if (nuevaContrasenaState.isBlank()) errorNuevaContrasena = "Campo obligatorio"
                        if (confirmarContrsenaState.isBlank()) errorConfirmar = "Campo obligatorio"
                        if (nuevaContrasenaState.isNotBlank() && confirmarContrsenaState.isNotBlank() && 
                            nuevaContrasenaState != confirmarContrsenaState) {
                            errorConfirmar = "Las contraseñas no coinciden"
                        }

                        if (errorCodigo.isEmpty() && errorNuevaContrasena.isEmpty() && errorConfirmar.isEmpty()) {
                            cargando = true
                            scope.launch {
                                repository.restablecerContrasena(NuevaContrasenaRequest(codigoState, nuevaContrasenaState))
                                    .onSuccess { response ->
                                    ...
                                    .onFailure { error ->
                                        Toast.makeText(context, error.message ?: "Error al restablecer contraseña", Toast.LENGTH_LONG).show()
                                        cargando = false
                                    }
                            }
                        }
